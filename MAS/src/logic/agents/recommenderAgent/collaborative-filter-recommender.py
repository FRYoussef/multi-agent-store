...
if __name__ == '__main__':
    ...
    try:
        ...
        views = customers.drop(
            ['Password', 'Preferences', 'Name', 'Gender', 'Purchases'], axis=1).dropna()

        purchases = customers.drop(
            ['Password', 'Preferences', 'Name', 'Gender', 'Views'], axis=1).dropna()
        ...
        if is_ok:  # If it will be a good recommendation
            ...
            # number of users with max kinship (3 < how_many < 5)
            how_many = min(n_users, len(ids_best_kinship))
            sorted(range(len(ids_best_kinship)),
                   key=lambda k: ids_best_kinship[k])  # sort ids by kinship
            print()
            ...
            for i in range(how_many):  # Get 1 random item of clothing of each user
                ...
                # If there's no purchases, search them in views (views always has items bc kinship > 1.3)
                if not item_list:
                    item_list = views['Views'][user_clothes].split(';')

                len_item_list = len(item_list)
                index_item = random.randrange(
                    0, len_item_list)  # Random index item
        ...
        if len(items) == 0:  # When the user viewed every random item checked -> random items
            for i in range(5):
